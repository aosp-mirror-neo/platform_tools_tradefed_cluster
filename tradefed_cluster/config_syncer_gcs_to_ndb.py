...
try:
  ...
  def _UpdateLabConfig(lab_name, lab_config_pbs, unified_lab_config):
    ...
    if not lab_info_entity:
      logging.debug('No lab info entity for %s, creating one.', lab_name)
      lab_info_entity = datastore_entities.LabInfo(
          id=lab_name, lab_name=lab_name
      )
      lab_info_entity.put()
    lab_config_entity = datastore_entities.LabConfig.get_by_id(lab_name)
    if not lab_config_entity:
      logging.debug('No lab config entity for %s, creating one.', lab_name)
      lab_config_entity = datastore_entities.LabConfig(
          id=lab_name, lab_name=lab_name, owners=owners
      )
  ...
  def _UpdateHostConfigs(host_to_host_config, host_to_unified_host_config):
    ...
    for hostname in hostnames:
      ...
      entities_from_config.append(
          _HostConfigEntityFromHostConfig(host_config, unified_host_config)
      )
  ...
  def SyncToNDB():
    ...
    lab_to_unified_lab_config = {
        lab_name: unified_lab_config_util.Parse(
            inventory_file, _GcsDataLoader()
        )
    ...
    lab_names = set(lab_to_unified_lab_config.keys()).union(
        lab_to_lab_pbs.keys()
    )
    for lab_name in lab_names:
      _UpdateLabConfig(
          lab_name,
          lab_to_lab_pbs.get(lab_name),
    ...
    # TODO: change to delete host config entities based on
    # existence.
  ...
  def _HostConfigEntityFromHostConfig(host_config, unified_host_config):
    ...
    if (
        unified_host_config
        and unified_host_config.GetVar(_GROUP_VAR_LAB_NAME)
        and lab_name == common.UNKNOWN_LAB_NAME
    ):
    ...
    if host_config:
      ...
      entity = datastore_entities.HostConfig(
          id=unified_host_config.name, hostname=unified_host_config.name
      )
  ...
  def _UpdateHostGroupConfigByInventoryData(lab_name, lab_config):
    ...
    logging.debug(
        'Loaded %d HostGroupConfigs of lab %s', len(entity_from_file), lab_name
    )
    entity_from_ndb = {
    ...
    for file in file_storage.ListFiles(_LAB_INVENTORY_DIR_PATH):
      if file.is_dir:
        ...
        if lab_dir_match and lab_dir_match.group(_LAB_NAME_KEY):
          yield (
              lab_dir_match.group(_LAB_NAME_KEY),
      ...
      for group in lab_config.ListGroups():
        ...
        if group.direct_vars.get(_OWNERS):
          acl_service.SyncUserPermissions(
              group_id, acl_service.Permission.owner, group.direct_vars[_OWNERS]
          )
...
def SyncGCSToNDB():
  ...
  if env_config.CONFIG.should_sync_lab_config:
    ...
    if ansible_import_error:
      logging.error(
          'Failed to import ansible package:\n%s', ansible_import_error
      )
